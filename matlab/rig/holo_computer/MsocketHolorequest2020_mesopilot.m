...
if ~isfield(holoRequest,'ignoreROIdata')  %if we're doing things normally  %ignoreROIdata existing is a 0 or 1 variable in Holorequest struct
  ...
  LN = size(holoRequest.targets,1);  %for blind mapping LN=216
    SLMCoordinates = zeros(4,LN);  
    SICoordinates = holoRequest.targets; %assign SI coordinates to targets fetched from HoloRequest struct size 216x3
    SICoordinates(:,1)=SICoordinates(:,1)+holoRequest.xoffset; % first column x corrected by xoffset
    SICoordinates(:,2)=SICoordinates(:,2)+holoRequest.yoffset;  % same for y  
    SICoordinates=SICoordinates'; %transform,now each row is a x,y,z dim and column is number of consecutive target
end
...
hiDE = AttenuationCoeffs>DEceil;
AttenuationCoeffs(hiDE)=DEceil;
SLMCoordinates(4,hiDE)=DEceil;
disp([num2str(sum(lowDE)) ' Target(s) below Diffraction Efficiency floor (' num2str(DEfloor) ').']);
disp([num2str(sum(hiDE)) ' Target(s) above Diffraction Efficiency ceiling (' num2str(DEceil) ').']);
DE_list(DE_list<DEfloor) = DEfloor;
DE_list(DE_list>DEceil) = DEceil;
