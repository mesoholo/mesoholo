...
% Relative path in repository: matlab/rig/daq/newStimParams_group_Uday_NULL_GALVOS.m
...
holoRequest.targets = [0,0,0];
holoRequest.actualtargets = holoRequest.targets;
holoRequest.groupID = [1];
holoRequest.roiWeights = [1];
holoRequest.oroiWeights = [1];
hSP.holoRequest = holoRequest;
...
powers           = [0.001]; %%%% Unordered LIST of Powers To Use in UNdivided mode (aka divided mode 1)
                                      %%%% If you want a no power condition, include 0 here,
                                      %%% [0 0.008 0.016 0.024 0.04]
                                      %%% [0 0.004 0.008 0.012 0.016 0.024]
                                      % 0.4 (0.025 multi, 0.3 multi9) for holeBurn @50 div, 0 0.02 0.04 0.08 0.12 for expt
pulseDurs        = [ 100 ]; %%%% LIST of pulse durations, 10 (5 multi) for holeBurn, 5 10 15 for expt
ipis             = [ 105 ]; % LIST of ipi in ms, compute from 1000/Hz, 20 (20 multi) for holeBurn, 15 30 45 for expt
nPulses          = [ 1 ]; % LIST of #pulses, 10 (100 for multi basler) for holeBurn, 2 5 for expt
...
hSP.nGroupsPerTrial  =  1; % 1 or Inf (Inf for stim test)
