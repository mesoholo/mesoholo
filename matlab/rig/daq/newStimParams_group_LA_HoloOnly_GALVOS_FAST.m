...
% Relative path in repository: matlab/rig/daq/newStimParams_group_LA_HoloOnly_GALVOS_FAST.m
...
powers           = [0 0.05]; %%%% Unordered LIST of Powers To Use in UNdivided mode (aka divided mode 1)
...
pulseDurs        = [ 10 ]; %%%% LIST of pulse durations, 10 (5 multi) for holeBurn, 5 10 15 for expt
ipis             = [ 100 ]; % LIST of ipi in ms, compute from 1000/Hz, 20 (20 multi) for holeBurn, 15 30 45 for expt
nPulses          = [ 10 ]; % LIST of #pulses, 10 (100 for multi basler) for holeBurn, 2 5 for expt
cellsPerHolo     = [ Inf ]; % LIST 1 (36 multi) for holeBurn, 1 15 for expt
...
hSP.multiplexFlag = 1; % SCALAR flag to set whether holograms are temporally multiplexed, 1 for long expts
...
if max(holoRequest.groupID)== 2 %sets safety multiplexing IPIs to match the galvo speed
    if hSP.multiplexFlag ==1
       hSP.deltaV0 = abs(holoRequest.AO0(2)-holoRequest.AO0(1));%Volt step for galvo X (in Volts)
       hSP.deltaV1 = abs(holoRequest.AO1(2)-holoRequest.AO1(1));%Volt step for galvo X (in Volts)
       hSP.deltaVmax = max(hSP.deltaV0,hSP.deltaV1);
       hSP.tmax = ceil(hSP.deltaVmax*13.5); %galvo speed, value specific to this set of galvos. In ms. 0.4deg/1200us, range 22.5deg, 5V.
       hSP.tmax = hSP.tmax+5;%5ms buffer added
       hSP.maxrate = 1000/hSP.tmax; %in Hz
       hSP.multiplexIPI = hSP.tmax;
       hSP.mplextotalstim = 2*pulseDurs + max(hSP.multiplexIPI,hSP.maxslmWait);
       hSP.mplexremain = ipis - hSP.mplextotalstim ;
       if hSP.mplexremain < hSP.tmax
           disp('Stim conditions not compatible with galvo speed')
           hSP = rmfield(hSP,'maxrate'); %So that the runner errors
       end
    end
end
...
if(hSP.multiplexFlag)%Added by LA 07/30/24- flag to collapse number of conditions when interleaving holos
allParams{1,5}= allParams{1,5}(1,1);
end
...
%Read Galvo voltages from MesoRequest for each stim condition
%hSP.voltage0 = zeros(1,size(hSP.weightID,2));
%hSP.voltage1 = zeros(1,size(hSP.weightID,2));
%for i=1:size(hSP.weightID,2)
    %curr_groupID = hSP.weightID(1,i);
    % curr_groupID == 0
        %hSP.voltage0(1,i)= 0;
        %hSP.voltage1(1,i)= 0;
    %else
       % hSP.voltage0(1,i)= holoRequest.AO0(1,curr_groupID);
        %hSP.voltage1(1,i)= holoRequest.AO1(1,curr_groupID);
        %hSP.powers(1,i) = hSP.powers(1,i)*holoRequest.powerfactors(1,curr_groupID);
    %end
%end
