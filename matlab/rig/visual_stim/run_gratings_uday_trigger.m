...
% Relative path in repository: matlab/rig/visual_stim/run_gratings_uday_trigger.m
% See README.md at repo root and docs/DEPENDENCIES.md for setup and hardware notes.
%

function run_gratings_uday_trigger(varargin)
p = inputParser;
p.addParameter('animalid','MU76_2');
p.addParameter('depth','180');
p.addParameter('repetitions',25); %240
...
p.addParameter('orientations',[0:30:150]);%[0,45,90,135,180,225,270,315]
p.addParameter('sFreqs',0.03); % cyc/vis deg %actual cpd/2 = sFreqs
p.addParameter('tFreqs',[3]); % cyc/sec
...
if ~result.do_triggered & condval>=1 & ~firstflag
...
%%%%% ALL THE INNER FXNS %%%%%
    function [thisstim, result] = setup_next(result, itrial, wininfo)
        thisstim = result.alltheseconds(:,itrial,result.cond_val);
